...
func (r *ReviewRepository) GetByStudio(ctx context.Context, studioID int64, limit, offset int) ([]domain.Review, error) {
...
func (r *ReviewRepository) SetOwnerResponse(ctx context.Context, reviewID int64, response string) (*domain.Review, error) {
	tx := r.db.WithContext(ctx).
		Table("reviews").
		Where("id = ?", reviewID).
