...
type UpdateStudioRequest struct {
	...
	Email        string                 `json:"email"`
	Website      string                 `json:"website"`
	WorkingHours domain.WorkingHoursMap `gorm:"type:jsonb" json:"working_hours,omitempty"`
...
type WorkingHoursResponse struct {
	...
	Hours        []domain.WorkingHours `json:"hours"`
...
func FormatCompactHours(hours []domain.WorkingHours) string {
	if len(hours) == 0 {
...
func CalculateLiveStatus(hours []domain.WorkingHours) (bool, string, string) {
	now := time.Now()
	dayOfWeek := int(now.Weekday()) // 0=Вс, 1=Пн, ...
	currentTime := now.Format("15:04")

	var todayHours *domain.WorkingHours
	for _, h := range hours {
...
func findNextOpenTime(hours []domain.WorkingHours, currentDay int) string {
	for i := 1; i <= 7; i++ {
