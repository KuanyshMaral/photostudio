...
func (h *Handler) ApproveStudio(c *gin.Context) {
	...
	if err != nil {
		response.CustomError(c, http.StatusBadRequest, "INVALID_ID", "Invalid studio owner ID")
		return
	}

	if err := h.service.ApproveStudioOwner(c.Request.Context(), studioOwnerID, adminID); err != nil {
		response.CustomError(c, http.StatusBadRequest, "APPROVE_ERROR", err)
		return
	}
...
func (h *Handler) RejectStudio(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		response.CustomError(c, http.StatusBadRequest, "VALIDATION_ERROR", "Reason is required")
		return
	}

	if err := h.service.RejectStudioOwner(c.Request.Context(), studioOwnerID, adminID, req.Reason); err != nil {
		response.CustomError(c, http.StatusBadRequest, "REJECT_ERROR", err)
		return
	}
...
func (h *Handler) BanUser(c *gin.Context) {
	...
	if err := h.service.BanUser(c.Request.Context(), userID, req.Reason); err != nil {
		response.CustomError(c, http.StatusBadRequest, "BAN_ERROR", err)
		return
	}
...
func (h *Handler) UnbanUser(c *gin.Context) {
	...
	if err := h.service.UnbanUser(c.Request.Context(), userID); err != nil {
		response.CustomError(c, http.StatusBadRequest, "UNBAN_ERROR", err)
		return
	}
...
func (h *Handler) HideReview(c *gin.Context) {
	...
	if err != nil {
		response.CustomError(c, http.StatusBadRequest, "INVALID_ID", "Invalid review ID")
		return
	}
