...
func (s *Service) RejectStudioOwner(ctx context.Context, studioOwnerID, adminID int64, reason string) error {
	...
	u.StudioStatus = auth.StatusRejected
	if err := s.userRepo.Update(ctx, u); err != nil {
		return err
	}
...
func (s *Service) GetPendingStudios(ctx context.Context, page, limit int) ([]catalog.Studio, int, error) {
	if page <= 0 {
		page = 1
	}
	if limit <= 0 || limit > 100 {
	...
	if err := s.studioRepo.DB().WithContext(ctx).
		...
		Where("u.studio_status = ? AND studios.deleted_at IS NULL", auth.StatusPending).
		Count(&total).Error; err != nil {
		return nil, 0, err
	}

	var studios []catalog.Studio
	if err := s.studioRepo.DB().WithContext(ctx).
		Table("studios").
		Joins("JOIN users u ON u.id = studios.owner_id").
		Where("u.studio_status = ? AND studios.deleted_at IS NULL", auth.StatusPending).
...
func (s *Service) VerifyStudio(ctx context.Context, studioID, adminID int64, notes string) (*catalog.Studio, error) {
	...
	owner.StudioStatus = auth.StatusVerified
...
func (s *Service) RejectStudio(ctx context.Context, studioID, adminID int64, reason string) (*catalog.Studio, error) {
	if strings.TrimSpace(reason) == "" {
	...
	owner.StudioStatus = auth.StatusRejected
	if err := s.userRepo.Update(ctx, owner); err != nil {
		return nil, err
	}
...
func (s *Service) GetPlatformStats(ctx context.Context) (map[string]interface{}, error) {
	...
	if err := s.userRepo.DB().WithContext(ctx).Table("users").Where("studio_status = ?", auth.StatusPending).Count(&pendingStudios).Error; err != nil {
		return nil, err
	}
	...
	if err := s.bookingRepo.DB().WithContext(ctx).
		Table("bookings").
		Where("status = ? AND updated_at >= ? AND updated_at < ?", booking.BookingCompleted, monthStart, monthEnd).
...
func (s *Service) BanUser(ctx context.Context, userID int64, reason string) error {
	...
	u.StudioStatus = auth.StatusBlocked
	if err := s.userRepo.Update(ctx, u); err != nil {
		return err
	}
...
func (s *Service) UnbanUser(ctx context.Context, userID int64) error {
	...
	if u.StudioStatus == auth.StatusBlocked {
		u.StudioStatus = auth.StatusVerified
	}
	return s.userRepo.Update(ctx, u)
}

func (s *Service) BlockUser(ctx context.Context, userID int64, reason string) (*auth.User, error) {
	u, err := s.userRepo.GetByID(ctx, userID)
	if err != nil {
		return nil, err
	}
...
func (s *Service) UnblockUser(ctx context.Context, userID int64) (*auth.User, error) {
	u, err := s.userRepo.GetByID(ctx, userID)
	if err != nil {
		return nil, err
	}
...
func (s *Service) ListUsers(ctx context.Context, filter UserListFilter, page, limit int) ([]auth.User, int, error) {
	if page <= 0 {
		page = 1
	}
	if limit <= 0 || limit > 100 {
	...
	if filter.Blocked != nil {
		if *filter.Blocked {
			q = q.Where("studio_status = ?", auth.StatusBlocked)
		} else {
			q = q.Where("(studio_status IS NULL OR studio_status <> ?)", auth.StatusBlocked)
		}
	}
	...
	// грузим в auth.User напрямую (у тебя json теги, а gorm по колонкам совпадает)
	var users []auth.User
	if err := q.
...
func (s *Service) ListReviews(ctx context.Context, filter ReviewListFilter, page, limit int) ([]review.Review, int, error) {
	if page <= 0 {
		page = 1
	}
	if limit <= 0 || limit > 100 {
...
func (s *Service) HideReview(ctx context.Context, reviewID int64) (*review.Review, error) {
...
func (s *Service) ShowReview(ctx context.Context, reviewID int64) (*review.Review, error) {
	rv, err := s.reviewRepo.GetByID(ctx, reviewID)
	if err != nil {
		return nil, err
	}
