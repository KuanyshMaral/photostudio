...
func ToUserBrief(u *auth.User) *UserBrief {
	if u == nil {
		return nil
	}
	return &UserBrief{
		ID:     u.ID,
		Name:   u.Name,
		Avatar: u.AvatarURL, // важно: в твоём auth.User поле AvatarURL
		Role:   string(u.Role),
	}
}
...
func ToConversationResponse(c *Conversation, currentUserID int64) *ConversationResponse {
	if c == nil {
		return nil
	}
