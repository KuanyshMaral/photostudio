...
type Service struct {
	...
	studioWorkingHoursRepo StudioWorkingHoursRepository
}
...
func (s *Service) CreateStudio(ctx context.Context, user *auth.User, req CreateStudioRequest) (*Studio, error) {
	// Check if user has permission
	if user.Role != auth.RoleStudioOwner || user.StudioStatus != auth.StatusVerified {
...
func (s *Service) UpdateStudio(ctx context.Context, userID, studioID int64, req UpdateStudioRequest) (*Studio, error) {
...
func (s *Service) GetStudiosByOwner(ctx context.Context, ownerID int64) ([]Studio, error) {
	return s.studioRepo.GetByOwnerID(ctx, ownerID)
}

func (s *Service) UpdateRoom(ctx context.Context, roomID int64, req UpdateRoomRequest) (*Room, error) {
	...
	if req.RoomType != nil {
		rt, err := ParseRoomType(*req.RoomType)
		if err != nil {
...
func (s *Service) CreateRoom(ctx context.Context, userID, studioID int64, req CreateRoomRequest) (*Room, error) {
	...
	roomType, err := ParseRoomType(req.RoomType)
	if err != nil {
		return nil, ErrInvalidRoomType
	}
...
func (s *Service) AddEquipment(ctx context.Context, userID, roomID int64, req CreateEquipmentRequest) (*Equipment, error) {
	// 1. Find the room
	room, err := s.roomRepo.GetByID(ctx, roomID)
	if err != nil {
		return nil, err
...
func (s *Service) calculateWorkingStatus(studio *Studio) *WorkingStatusResponse {
...
func (s *Service) UpdateStudioWorkingHours(ctx context.Context, userID, studioID int64, hours []WorkingHours) error {
	studio, err := s.studioRepo.GetByID(ctx, studioID)
	if err != nil {
		return err
	}
	...
	studioHours := &StudioWorkingHours{
		StudioID: studioID,
...
func (s *Service) GetWorkingHoursForDate(ctx context.Context, studioID int64, date time.Time) (*WorkingHours, error) {
	hours, err := s.studioWorkingHoursRepo.GetHoursForStudio(studioID)
	if err != nil {
		return nil, err
	}
