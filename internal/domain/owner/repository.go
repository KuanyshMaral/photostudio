...
func (r *OwnerRepository) Create(ctx context.Context, owner *StudioOwner) error {
...
func (r *OwnerRepository) FindByID(ctx context.Context, id int64) (*StudioOwner, error) {
	var owner StudioOwner
...
func (r *OwnerRepository) Update(ctx context.Context, owner *StudioOwner) error {
	return r.db.WithContext(ctx).
		Model(&StudioOwner{}).
		Where("id = ?", owner.ID).
...
func (r *OwnerRepository) FindPendingPaginated(ctx context.Context, offset, limit int) ([]PendingStudioOwnerRow, int64, error) {
	var total int64
	if err := r.db.WithContext(ctx).
		Table("studio_owners so").
		Joins("JOIN users u ON u.id = so.user_id").
		Where("u.studio_status = ?", "pending").
		Count(&total).Error; err != nil {
		return nil, 0, err
	}

	var rows []PendingStudioOwnerRow
	if err := r.db.WithContext(ctx).
		Table("studio_owners so").
