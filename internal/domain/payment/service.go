...
func (s *Service) HandleFailCallback(ctx context.Context, outSum string, invID int64, signature string, shpParams map[string]string) error {
	if !strings.EqualFold(signature, s.generateSignatureForSuccess(outSum, invID, shpParams)) {
		return ErrInvalidSignature
	}
	if s.repo != nil {
		p, err := s.repo.GetPaymentByInvoiceID(ctx, invID)
	...
	return s.payments.UpdateStatus(ctx, invID, RobokassaPaymentStatus(booking.PaymentUnpaid), "", "failed callback", nil)
}
