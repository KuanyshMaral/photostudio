...
func TestRouteRegistrationsAndHelpers(t *testing.T) {
	...
	h.RegisterPublicCallbackRoutes(p)
	...
	w = httptest.NewRecorder()
	req = httptest.NewRequest(http.MethodPost, "/payments/robokassa/success", nil)
	r.ServeHTTP(w, req)
	if w.Code != http.StatusBadRequest {
		t.Fatalf("expected success callback bad request got %d", w.Code)
	}
