...
func TestGenerateInvoiceID_JSNumberSafeAndMonotonic(t *testing.T) {
	const jsMaxSafeInt = int64(9007199254740991) // 2^53-1
	a := generateInvoiceID()
	b := generateInvoiceID()
	if a <= 0 || b <= 0 {
		t.Fatalf("invoice ids must be positive: a=%d b=%d", a, b)
	}
	if a >= jsMaxSafeInt || b >= jsMaxSafeInt {
		t.Fatalf("invoice ids must be JS-safe numbers: a=%d b=%d", a, b)
	}
	if b <= a {
		t.Fatalf("invoice ids must increase within process: a=%d b=%d", a, b)
	}
}
