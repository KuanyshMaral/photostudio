...
type Handler struct {
	service          *Service
	workingHoursRepo catalog.StudioWorkingHoursRepository // добавить
}
...
func (h *Handler) GetStudioBookings(c *gin.Context) {
	...
	if err != nil {
		response.CustomError(c, http.StatusInternalServerError, "FETCH_FAILED", "Failed to get bookings")
		return
	}
...
func (h *Handler) UpdatePaymentStatus(c *gin.Context) {
	...
	if err != nil {
		if errors.Is(err, ErrForbidden) {
			response.CustomError(c, http.StatusForbidden, "FORBIDDEN", "You cannot update this booking")
			return
		}
		response.CustomError(c, http.StatusInternalServerError, "UPDATE_FAILED", "Failed to update payment status")
		return
	}
...
func (h *Handler) ConfirmBooking(c *gin.Context) {
	...
	if err != nil || !isOwner {
		response.CustomError(c, http.StatusForbidden, "FORBIDDEN", "Only studio owner can confirm")
		return
	}
...
func (h *Handler) CancelBooking(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		response.CustomError(c, http.StatusBadRequest, "VALIDATION_ERROR",
	...
	if err != nil {
		response.CustomError(c, http.StatusNotFound, "NOT_FOUND", "Booking not found")
		return
	}
	...
	if !canCancel {
		...
		if !isOwner {
			response.CustomError(c, http.StatusForbidden, "FORBIDDEN", "Cannot cancel this booking")
			return
		}
	}
	...
	if booking.Status == BookingCompleted {
		response.CustomError(c, http.StatusBadRequest, "INVALID_STATUS", "Cannot cancel completed booking")
		return
	}

	if booking.Status == BookingCancelled {
		response.CustomError(c, http.StatusBadRequest, "INVALID_STATUS", "Booking is already cancelled")
		return
	}
	...
	if err != nil {
		response.CustomError(c, http.StatusBadRequest, "CANCEL_ERROR", err)
		return
	}
...
func (h *Handler) CompleteBooking(c *gin.Context) {
	...
	isOwner, err := h.service.IsBookingStudioOwner(c.Request.Context(), userID, bookingID)
	if err != nil || !isOwner {
		response.CustomError(c, http.StatusForbidden, "FORBIDDEN", "Only studio owner can complete")
		return
	}
	...
	if booking.Status != "confirmed" {
		response.CustomError(c, http.StatusBadRequest, "INVALID_STATUS", "Can only complete confirmed bookings")
		return
	}
...
func (h *Handler) MarkBookingPaid(c *gin.Context) {
	...
	userID := c.GetInt64("user_id")
	if userID == 0 {
		response.CustomError(c, http.StatusUnauthorized, "UNAUTHORIZED", "Missing auth")
		return
	}

	b, err := h.service.UpdatePaymentStatus(c.Request.Context(), bookingID, userID, PaymentPaid)
	if err != nil {
		if errors.Is(err, ErrForbidden) {
			response.CustomError(c, http.StatusForbidden, "FORBIDDEN", "You cannot update this booking")
			return
...
func (h *Handler) UpdateDeposit(c *gin.Context) {
	...
	if userRole != "admin" && userRole != "studio_owner" {
		response.CustomError(c, http.StatusForbidden, "FORBIDDEN", "Access denied")
		return
	}
